...
class SpeechRecognizer: ObservableObject {
    ...
    // SherpaOnnx offline recognizer — no network required
    private let sherpaOnnx = SherpaOnnxRecognizer()
    private var sherpaOnnxCancellable: AnyCancellable?
    private var modelDir: String { Bundle.main.resourcePath ?? "" }
    ...
    /// Feed audio from AVCaptureSession (recording) into sherpa-onnx recognizer
    func feedAudioSampleBuffer(_ sampleBuffer: CMSampleBuffer) {
        guard isListening, sherpaOnnx.isReady else { return }
        sherpaOnnx.feedAudioSampleBuffer(sampleBuffer)
    }
    ...
    func beginRecognitionFromRecordingAudio() {
        ...
        if !sherpaOnnx.isReady {
            do { try sherpaOnnx.start(modelDir: modelDir) }
            catch { self.error = "语音模型加载失败: \(error.localizedDescription)"; isListening = false; return }
        }

        let currentGeneration = sessionGeneration

        sherpaOnnxCancellable = sherpaOnnx.$partialText
            .receive(on: DispatchQueue.main)
            .sink { [weak self] spoken in
                guard let self, self.sessionGeneration == currentGeneration, !spoken.isEmpty else { return }
                print("🎤 识别: 「\(spoken)」→ offset=\(self.recognizedCharCount)")
    ...
    private func cleanupRecognition() {
        sherpaOnnxCancellable?.cancel()
        sherpaOnnxCancellable = nil
        sherpaOnnx.reset()
        asr.stop()
    }
    ...
    private func beginRecognition() {
        ...
        // Load sherpa-onnx model if not already loaded
        if !sherpaOnnx.isReady {
            do { try sherpaOnnx.start(modelDir: modelDir) }
            catch { self.error = "语音模型加载失败: \(error.localizedDescription)"; isListening = false; return }
        }
        ...
        // Feed captured audio to sherpa-onnx
        asr.onAudioBuffer = { [weak self] buffer in
            guard let self, self.sessionGeneration == currentGeneration, self.isListening else { return }
            guard let channelData = buffer.floatChannelData else { return }
            let frames = Int(buffer.frameLength)
            let samples = Array(UnsafeBufferPointer(start: channelData.pointee, count: frames))
            self.sherpaOnnx.acceptWaveform(samples: samples, sampleRate: Int32(buffer.format.sampleRate))
        }

        // Subscribe to sherpa-onnx partial results
        sherpaOnnxCancellable = sherpaOnnx.$partialText
            .receive(on: DispatchQueue.main)
            .sink { [weak self] spoken in
                guard let self, self.sessionGeneration == currentGeneration, !spoken.isEmpty else { return }
