...
class NotchFrameTracker: ObservableObject {
    ...
    @Published var visibleWidth: CGFloat = 200 {
        didSet { updatePanel() }
    }
...
class OverlayContent: ObservableObject {
    ...
    @Published var pagePreviews: [String] = []
    @Published var showPagePicker: Bool = false
    @Published var jumpToPageIndex: Int? = nil
}
...
class NotchOverlayController: NSObject {
    ...
    func show(text: String, hasNextPage: Bool = false, onComplete: (() -> Void)? = nil) {
        showOverlay(
            ...
            startsSpeechRecognizer: true,
            isPreviewOnly: false
        )
    }

    func showPreview(text: String, hasNextPage: Bool = false) {
        showOverlay(
            text: text,
            hasNextPage: hasNextPage,
            onComplete: nil,
            startsSpeechRecognizer: false,
            isPreviewOnly: true
        )
    }
    ...
    private func showOverlay(
        ...
        hasNextPage: Bool,
        onComplete: (() -> Void)?,
        startsSpeechRecognizer: Bool,
        isPreviewOnly: Bool
    ) {
        self.onComplete = onComplete
        self.onNextPage = isPreviewOnly ? nil : { [weak self] in
            CuteRecordService.shared.advanceToNextPage()
        }
        ...
        if settings.overlayMode == .fullscreen {
            ...
            showFullscreen(settings: settings, screen: fsScreen, installsKeyMonitor: !isPreviewOnly, isPreviewOnly: isPreviewOnly)
        } else if settings.overlayMode == .floating && settings.followCursorWhenUndocked {
            showFollowCursor(settings: settings, screen: screen, installsKeyMonitor: !isPreviewOnly, isPreviewOnly: isPreviewOnly)
        } else {
            switch settings.overlayMode {
            case .pinned:
                showPinned(settings: settings, screen: screen, installsKeyMonitor: !isPreviewOnly, isPreviewOnly: isPreviewOnly)
            case .floating:
                showFloating(settings: settings, screenFrame: screenFrame, installsKeyMonitor: !isPreviewOnly, isPreviewOnly: isPreviewOnly)
            case .fullscreen:
                break // handled above
            }
        }
        ...
        if !isPreviewOnly && settings.overlayMode == .floating && settings.followCursorWhenUndocked {
            showStopButton(on: screen)
        }

        // Word tracking & silence-paused need the microphone; classic does not
        if startsSpeechRecognizer && settings.listeningMode != .classic {
    ...
    private func showPinned(settings: NotchSettings, screen: NSScreen, installsKeyMonitor: Bool = true, isPreviewOnly: Bool = false) {
        ...
        let overlayView = NotchOverlayView(content: overlayContent, speechRecognizer: speechRecognizer, menuBarHeight: menuBarHeight, baseTextHeight: textAreaHeight, maxExtraHeight: maxExtraHeight, frameTracker: tracker, isPreviewMode: isPreviewOnly)
    ...
    private func showFollowCursor(settings: NotchSettings, screen: NSScreen, installsKeyMonitor: Bool = true, isPreviewOnly: Bool = false) {
    ...
    private func showFullscreen(settings: NotchSettings, screen: NSScreen, installsKeyMonitor: Bool = true, isPreviewOnly: Bool = false) {
        let screenFrame = screen.frame
    ...
    private func showFloating(settings: NotchSettings, screenFrame: CGRect, installsKeyMonitor: Bool = true, isPreviewOnly: Bool = false) {
        let panelWidth = settings.notchWidth
        let panelHeight = settings.textAreaHeight
        ...
        Timer.publish(every: 0.1, on: .main, in: .common)
            ...
            .sink { [weak self] _ in
                ...
                if let targetIndex = self.overlayContent.jumpToPageIndex {
                    ...
                    CuteRecordService.shared.jumpToPage(index: targetIndex)
                }
...
struct NotchOverlayView: View {
    ...
    private func charOffsetForWordProgress(_ progress: Double) -> Int {
        guard progress.isFinite, progress < Double(Int.max) else { return 0 }
    ...
    var body: some View {
        ...
        .onReceive(scrollTimer) { _ in
            guard !isDone, !isUserScrolling, scrollEnabled, !isPreviewMode else { return }
    ...
    private var pagePickerView: some View {
        ScrollView(.vertical, showsIndicators: false) {
            VStack(alignment: .leading, spacing: 6) {
                ...
                Text(uiText("Tap a page to jump"))
                    .font(.system(size: 10))
